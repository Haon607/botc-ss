...
export class Character implements Verifiable {
    ...
    verify(): Errors {
        ...
        if (this.id.trim() === '')
            err.messages.push(new Message('Field id empty', Severity.ERROR, this));
        if (this.name.trim() === '')
            err.messages.push(new Message('Field name empty', Severity.ERROR, this));
        if (this.image?.trim() === '')
            err.messages.push(new Message('Field image empty', Severity.ERROR, this));
        if (this.ability.trim() === '')
            err.messages.push(new Message('Field ability empty', Severity.ERROR, this));
        if (this.flavour.trim() === '')
            err.messages.push(new Message('Field flavour empty', Severity.WARN, this));
        if (this.reminders.some((reminder) => reminder.trim() === ''))
            err.messages.push(
                new Message('Field reminders contains an empty entry', Severity.WARN, this)
            );
        if (this.remindersGlobal.some((reminder) => reminder.trim() === ''))
            err.messages.push(
                new Message('Field remindersGlobal contains an empty entry', Severity.WARN, this)
            );
        ...
        if (this.otherNight === 0 && this.otherNightReminder)
            err.messages.push(
                new Message(
                    'Field otherNIghtReminder is set, although field otherNight is 0',
                    Severity.WARN,
                    this,
                ),
            );
        ...
        if (
            ...
            err.messages.push(
                new Message('If this Character kills in the Night, consider adding a "DEAD" Reminder-Token to the Field reminders ', Severity.INFO, this)
            );
